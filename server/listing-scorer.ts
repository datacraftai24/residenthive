...
export class ListingScorer {
  ...
  categorizeListings(scoredListings: ScoredListing[]): CategorizedListings {
    ...
    // Categorize properties WITH images using score thresholds (0-1 scale)
    const topPicks = propertiesWithImages.filter(item => item.match_score >= 0.35); // 35%
    const otherMatches = propertiesWithImages.filter(item => item.match_score >= 0.25 && item.match_score < 0.35); // 25-35%
